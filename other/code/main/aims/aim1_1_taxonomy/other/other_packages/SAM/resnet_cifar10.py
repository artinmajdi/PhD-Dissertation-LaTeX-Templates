...
def learner(x, n_blocks):
    """ Construct the Learner
        thresh_technique          : input to the learner
...
def residual_group(x, n_filters, n_blocks, strides=(2, 2), n=2):
    """ Construct a Residual Group
        thresh_technique         : input into the group
...
def identity_block(x, n_filters, n=2):
    """ Construct a Bottleneck Residual Block of Convolutions with Identity Shortcut
        thresh_technique        : input into the block
...
def classifier(x, n_classes, activation):
    ''' Construct the Classifier
        thresh_technique         : input into the classifier
    ...
    outputs = Dense(n_classes, activation=activation, kernel_initializer='he_normal')(x)
    return outputs
...
outputs = classifier(x, 10, 'softmax')
