...
% Our approach is distinguished by its application of a weighted soft majority voting scheme, where the weights are determined based on the level of uncertainty associated with each annotator's labels. Importantly, the proposed technique takes into account the possibility of consistently inaccurate annotators and includes measures to penalize them, thus ensuring the credibility of the computed weights. The employment of an ensemble of classifiers for each annotator further bolsters the robustness of the technique.

% Moreover, the crowd-certain technique demonstrates an appreciable capability to generate confidence scores that accompany each aggregated label, offering an extended context that can be invaluable in practical applications. The calculated weights follow a pre-set ground-truth accuracy closely, highlighting the effectiveness of the technique in capturing the quality of annotators' labels.

% The weight measurement evaluation using two different strategies indicated a slight preference for the second strategy due to its lower computational requirements and reduced dependencies on Python packages. The visualization of the relationship between an annotator's probability threshold and their assigned weight indicated that our proposed method manifests a stronger correlation than Tao's method, further validating its efficacy.

% Significantly, our technique introduces an advantageous property by assigning a single weight to each annotator for all instances in the dataset. This implies that the model can label new instances without having to recalculate annotator weights each time. In practical applications, where annotators may be scarce or where continuous recalculation may not be feasible, this is of substantial benefit.

% Our results show that the crowd-certain method offers not only comparable performance to other techniques in scenarios with a larger number of annotators but also a noteworthy improvement in accuracy where annotator numbers may be limited. This supports the scalability and practical applicability of our technique in diverse contexts.

% In summary, our proposed crowd-certain method presents a significant advancement in label aggregation techniques for crowdsourcing and ensemble learning applications. It offers enhanced accuracy, flexibility, and efficiency, thus holding considerable potential for adoption in a range of real-world scenarios. Future work may seek to refine the technique further and to explore its applications in more complex or larger-scale datasets.

% In this study, we evaluated various techniques for aggregating crowd-sourced labels and measuring the confidence scores associated with these labels. This evaluation involved two key metrics: Expected Calibration Error (ECE) and Brier Score Loss, as well as three additional metrics: accuracy, AUC, and F1 score. These metrics assessed different facets of model performance: calibration and sharpness of the confidence scores, and the performance of the aggregated labels against the ground truth.

% Our results indicate that the choice of aggregation and confidence measurement technique can significantly impact the performance. Specifically, the Crowd-Certain technique consistently performed well when combined with the frequentist strategy, yielding lower ECE and Brier Score Loss across different numbers of labelers. This suggests that the Crowd-Certain technique can effectively aggregate crowd-sourced labels and produce well-calibrated and sharp confidence scores when applied using the frequentist strategy.

% We also noted that increasing the number of labelers does not necessarily improve the performance, as indicated by the general increase in ECE and Brier Score Loss with a higher number of labelers. This suggests a trade-off between the number of labelers and the performance, and that the optimal number may depend on the specific context and the chosen techniques.

% We implemented additional performance metrics (accuracy, AUC, and F1 score) to evaluate the quality of the aggregated labels in comparison to the ground truth. It's crucial to note that while these metrics are not directly related to the confidence scores, they provide essential insights into the effectiveness of the aggregation process.

% Future work could delve deeper into understanding why certain techniques and strategies outperform others under specific conditions. Further investigations could explore the effects of other factors such as the complexity of the task and the diversity of the crowd, which may impact the performance of different techniques and strategies.

% In conclusion, the choice of aggregation and confidence measurement technique plays a significant role in effectively leveraging crowd-sourced labels. Our findings could guide future research and applications in this domain, with potential implications for various fields that rely on crowd-sourced data, including machine learning, data science, and citizen science.

Label aggregation is a critical component of crowdsourcing and ensemble learning strategies. Many generic label aggregation algorithms fall short because they do not account for the varying reliability of the annotators. In response to this, we have developed a novel label aggregation method that calculates a reliability score for each annotator based on the annotator's consistency versus a trained classifier. In the first approach (proposed techniques without penalization), we utilize uncertainty estimates to assign each annotator a more accurate weight, which correlates with their agreement with others and their consistency during labeling. In the second approach (proposed technique with penalization. Also noted as crowd-certain), we improve  on this technique by penalizing the annotator for their disagreements with other annotators (shown in Eq.~\ref{eq:crowd.eq.10.consistency-penalized} and hence mitigate the effect of annotator's bias when calculating the final weights $\omega_{\alpha}^{(k)}$).
