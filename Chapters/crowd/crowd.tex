...
a worker may be skilled at labeling dogs, but not rabbits.) Then we use these probability thresholds to generate the crowd label set $Z_{\alpha}^{(i)} $ from the ground truth labels for each instance $i $.
...
\begin{itemize}
    ...
    \end{enumerate}
    \item Let's Define $t_{\alpha}^{(i,k),(g)} $ as the predicted label obtained by binarizing the predicted probabilities $p_{\alpha}^{ (i,k),(g)} $ using the threshold $\theta_{\alpha}^{(k),(g)} $ as shown in the Glossary of Symbols section.
...
\end{enumerate}
The confidence score serves as a quantitative measure of the reliability of the classification of instance `i' into class `k'.
The CDF of the beta distribution function can be defined as follows~\cite{BetaDistributionWikipedia}:
\begin{equation}
...
The CDF of the beta distribution at the decision threshold of $x=0.5 $ (denoted as $I_{0.5}$) is used to calculate a confidence score $F_{\beta}^{(i, k)}$. To calculate I_0.5, we first need to calculate two shape parameters $l^{(i, k)}$ and $u^{(i, k)}$
