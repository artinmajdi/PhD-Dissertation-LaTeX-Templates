...
< Reviewed Until Here >
% BF10: BF10 stands for Bayes Factor. Bayes Factor is a statistical measure that quantifies the evidence for one statistical model over another model, given the data. BF10 specifically compares the evidence for an alternative hypothesis (H1) versus a null hypothesis (H0). A BF10 of 1 indicates that the data are equally likely under both the null and alternative hypotheses. A BF10 greater than 1 indicates evidence favoring H1, and less than 1 indicates evidence favoring H0. Generally, values of 3 and above are considered some evidence for H1, values below 1/3 as evidence for H0, and between 1/3 and 3 as inconclusive.

% Cohen's d: Cohen's d is a measure of effect size used in the context of a t-test for the difference between two means. It's defined as the difference between two means divided by a standard deviation for the data. Cohen's d values of 0.2, 0.5, and 0.8 are typically considered to represent small, medium, and large effect sizes, respectively. It's a standard way of quantifying the difference between two groups that has been corrected for differences in sample size and variance.

% Statistical Power: Power is the probability that a statistical test will correctly reject the null hypothesis when the alternative hypothesis is true (i.e., the test will not make a Type II error). Power is generally desired to be 0.8 or higher, which means there is an 80% or greater chance of detecting a true effect if it exists. Power can be influenced by a number of factors including the effect size, sample size, significance level, and the variability in the data. Calculating power can be used in planning a study, to determine the sample size needed to detect an effect of a given size.
