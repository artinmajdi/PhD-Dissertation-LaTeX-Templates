...
The selection of appropriate hyperparameters is crucial for the effectiveness of the proposed transfer learning technique. In this study, we employ a systematic approach to tune the hyperparameters $\alpha_k $, which controls the dependency between the predicted probabilities of the child and parent classes. We utilize a grid search method along with cross-validation to determine the optimal values for these hyperparameters. The search space for both hyperparameters is defined based on preliminary experiments and domain knowledge, ensuring a balance between model complexity and predictive performance.

\begin{enumerate}
    \item Combine all three datasets (NIH, CheXpert, PADCHEST). Let's denote this dataset as $\mathcal{D} = (X, Y)$
    \item Split the new dataset $\mathcal{D}$ into three subsets: $\mathcal{D}^{\text{train}}$, $\mathcal{D}^{\text{valid}}$, and $\mathcal{D}^{\text{test}}$, representing $60\%$, $10\%$, and $30\%$ of the data respectively.
    \item Optimize the CNN model (DenseNet121) $f_1(\cdot)$, using $\mathcal{D}^{\text{train}}$ and $\mathcal{D}^{\text{valid}}$.
    \item Create a secondary model $f_2 (\cdot)$:
    \begin{itemize}
        \item Construct a second model $f_2(\cdot)$ which is identical in architecture to the original model $f(\cdot)$ except for the last activation layer (sigmoid).
        \item Update the weights and biases in the $f_2 (\cdot)$ using their counterpart values in $f(\cdot)$
    \end{itemize}
    \item Obtain the logit values:
    \begin{itemize}
        \item Run the data samples $x^{(i)}$ in both $\mathcal{D}^{\text{train}}$ and $\mathcal{D}^{\text{test}}$ subsets as test cases into the trained model $f_2(\cdot)$ to obtain $K$ logit values $Q = \{q_k^{(i)}\}_{k=1}^{K}$ for $K$ classes for each instance $i$, where $Q = f_2(x^{(i)})$.
        \item Define $\mathcal{T}^{\text{train}} = \{X^{\text{train}}, Y^{\text{train}}, Q^{\text{train}}\}$ and $\mathcal{T}^{\text{test}} = \{X^{\text{test}}, Y^{\text{test}}, Q^{\text{test}}\}$.
    \end{itemize}
    \item Perform Hyper Parameter Tuning to find the $\alpha_k$ parameter values:
    \begin{itemize}
        \item Use $\mathcal{T}^{\text{train}}$ to determine the instances of $\alpha_k$ where the Area Under the Curve (AUC) is maximized.
        \begin{itemize}
            \item \textbf{Search Space:} A uniform distribution $U(-1, 1)$.
            \item \textbf{Optimization Algorithm:} Tree-structured Parzen Estimator (TPE). TPE is a sequential model-based optimization (SMBO) approach.
            \item A Bayesian optimization strategy is employed where it iteratively constructs a probabilistic model of the function to be optimized and uses this model to select the most promising parameters to evaluate in the true function.
            \item This algorithm is generally considered to be more efficient than other optimization methods such as grid search and random search, especially for high-dimensional hyperparameter optimization problems. This is due to its ability to focus the search on promising areas of the parameter space, guided by the information collected from previous evaluations of the objective function.
        \end{itemize}
        \item Use the calculated $\alpha_k$ to find the updated predicted probability values for instances in $\mathcal{T}^{\text{test}}$.
    \end{itemize}
    \item Repeat step 6 for different thresholding techniques (ROC, AUC, 0.5).
\end{enumerate}

% \begin{algorithm}
%     \caption{Combining datasets and logit adjustment}
%     \begin{enumerate}
%         \item Combine all three datasets (NIH, CheXpert, PADCHEST). Let's denote this dataset as $\mathcal{D} = (X, Y)$
%         \item Split the new dataset $\mathcal{D}$ into three subsets: $\mathcal{D}^{\text{train}}$, $\mathcal{D}^{\text{valid}}$, and $\mathcal{D}^{\text{test}}$, representing $60%$, $10%$, and $30%$ of the data respectively.

%         \item Optimize the CNN model $f_1(\cdot)$, using $\mathcal{D}^{\text{train}}$ and $\mathcal{D}^{\text{valid}}$.
%         \item Create a secondary model $f_2(\cdot)$:
%         \begin{itemize}
%             \item Construct a second model $f_2(\cdot)$ which is identical in architecture to the original model $f(\cdot)$ except for the last activation layer (sigmoid).
%             \item Update the weights and biases in the $f_2(\cdot)$ using their counterpart values in $f(\cdot)$
...
\begin{SgAlgorithm}
\caption{Training the Classification Model $f_1 (\cdot)$}
\label{alg:taxonomy.predictor}
\begin{algorithmic}
    \State{Combine all three datasets (NIH, CheXpert, PADCHEST) into one dataset called $\mathcal{D} = (X, Y)$}

    \State{Randomly split the $\mathcal{D}$ into $\mathcal{D}^\text{train}$, $\mathcal{D}^\text{valid}$, and $\mathcal{D}^\text{test}$ subsets, representing \%60, \%10, and \%30 of the data}

    \State{Optimize the predictor model (DenseNet121) $f_1(\cdot)$, using $\mathcal{D}^\text{train}$ and $\mathcal{D}^\text{valid}$.}

    \State{Construct $f_2(\cdot)$ predictor model using an architecture identical to $f(\cdot)$ except for the last layer (Sigmoid layer)}

    \State{Update weights and biases in $f_2 (\cdot)$ using their counterparts in $f_1 (\cdot)$}
\end{algorithmic}
\end{SgAlgorithm}

% \TODO complete the below algorithms

% \begin{SgAlgorithm}
% \caption{Applying the ``logit_based'' technique Algorithm}
% \label{alg:taxonomy.logit}
% \begin{algorithmic}
%     \State{Get the predictor model $f_2(\cdot)$ from Algorithm~\ref{alg:taxonomy.predictor}}
%     \State{Feed the data instances $x^{(i)}$ in both $\mathcal{D}^\text{train}$ and $\mathcal{D}^\text{test}$ as test cases, into the $f_2(\cdot)$ predictor to obtain $K$ logit values $Q = {q_k^{(i)}}_{k=1}^{K}$}

%     \State{Define $\mathcal{T}^{\text{train}} = {X^{\text{train}}, Y^{\text{train}}, Q^{\text{train}}}$ and $\mathcal{T}^{\text{test}} = {X^{\text{test}}, Y^{\text{test}}, Q^{\text{test}}}$}

%     \State{Get the $\alpha_k$ from Algorithm~\ref{alg:taxonomy.hyperparameter_tuning}}
%     % \State{Use $\alpha_k$ and Eq~\ref{} }
% \end{algorithmic}
% \end{SgAlgorithm}
...
% \label{alg:taxonomy.hyperparameter_tuning}
% \begin{algorithmic}
%     \State{For each instance of $\alpha_k$ in $\mathcal{T}^\text{train}$
%         \State{Define search space as $U(-1, 1)$}
%         \State{Employ Bayesian optimization strategy: Tree-structured Parzen Estimator (TPE)}
%         \State\If{$\alpha_k$ maximizes Area Under the Curve (AUC)}{
%             \State{Update predicted probability values for instances in $\mathcal{T}^{\text{test}}$ using calculated $\alpha_k$}}
%         }
%     \State\For{\text{each thresholding technique (ROC, AUC, 0.5)}}{
%         \State{Repeat step 6}
%     }
% \end{algorithmic}
% \end{SgAlgorithm}
